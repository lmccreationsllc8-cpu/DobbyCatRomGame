...
Idle quit and **hold Select** (~1.25s) exit with code **0** so RCARS restores attract. Mid-run **PAUSE** chip (tap) or **Start** tap opens the pause menu (Continue / Return to title / Quit). See RCARS `docs/runbook.md` § Booth Blaster.
...
No extra setup: Cross fires; Share hold quits the app; Options confirms / Start tap opens pause.
